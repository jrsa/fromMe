...
shader::shader(const GLchar *vs_src, const GLchar *fs_src) {
  ...
  glGetProgramiv(_program, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &max_attr_namelength);
  ...
  for (int attr = 0; attr < n_active_attr; ++attr) {
    ...
    GLenum type = GL_FLOAT;
    GLsizei actualLength = 0;
    glGetActiveAttrib(_program, attr, nameData.size(), &actualLength, &arraySize, &type, &nameData[0]);
